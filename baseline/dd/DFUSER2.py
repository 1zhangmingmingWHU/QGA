...
class DiT1dDiffusion(nn.Module):
    def __init__(
        self, 
        emb_dim: int,
        horizon: int,
        ...
        self.map_noise = self._get_timestep_embedding(timestep_emb_type, emb_dim, **timestep_emb_params)
        
        # Position embedding
        self.pos_emb = nn.Embedding(horizon, emb_dim)
        self.pos_emb_cache = None
        
        # Input projection
        self.x_proj = nn.Linear(in_dim, emb_dim)
        ...
        # Transformer blocks (placeholder, you'd need to define the actual block structure)
        self.blocks = nn.ModuleList([
            TransformerBlock(emb_dim) for _ in range(num_blocks)
        ])
        
        # Final layer
        self.final_layer = FinalLayer(emb_dim, in_dim)
        
        # Weight initialization
        self.initialize_weights()
    
    def _get_timestep_embedding(self, emb_type, emb_dim, **params):
        # Implement timestep embedding selection logic
        # This is a placeholder and should be replaced with actual implementation
        if emb_type == "positional":
            return PositionalEmbedding(emb_dim, **params)
        else:
            raise ValueError(f"Unsupported timestep embedding type: {emb_type}")
    ...
    def initialize_weights(self):
        ...
        # Time step embedding initialization
    ...
    def forward(
        ...
        # Update position embedding cache if needed
...
# ------------------------------------------ sampling ------------------------------------------#
    ...
    def loss(self, x, cond, returns, masks):
        ...
        batch_size = len(x)     # x = torch.cat([actions, states], dim=-1)
        t = torch.randint(0, self.n_timesteps, (batch_size,), device=x.device).long()

        diffuse_loss, info = self.p_losses(x[:, :, self.action_dim:], cond, t, returns, masks)  # state,state,t,returns,masks
...
class DFUSER(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        cur_time = cur_time + 1
        states = x[:cur_time]
        states = states[:, :-1] # 移除timeStepIndex
        conditions = states
        returns = torch.tensor([[1.0]], device=x.device) # return 直接设为1
        x_0 = self.diffuser(cond=conditions, returns=returns)

        states = x_0[0, :cur_time + 1]  # 获取生成的状态
        states_next = states[None, -1]  # 提取最后一个状态作为下一个状态
        if cur_time > 1:
    ...
    def load_net(self, load_path="saved_model/fixed_initial_budget", device='cuda:0'):
        self.diffuser.load_state_dict(torch.load(load_path, map_location=device))
