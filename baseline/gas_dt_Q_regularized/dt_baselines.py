...
class Critic(nn.Module):
    ...
    def save_net(self, save_path):
        ...
        file_path = os.path.join(save_path, 'dt_Q_regularized_critic.pt')
...
class DecisionTransformer(nn.Module):
    ...
    def step(self, states, actions, rewards, dones, rtg, timesteps, attention_mask, ctg, score_to_go, costs):
        ...
        _, action_preds, state_preds, _ = self.forward(
    ...
    def save_net(self, save_path):
        if not os.path.exists(save_path):
            os.makedirs(save_path)
        file_path = os.path.join(save_path, 'dt_reweight_w_0_2_scale_50_Q_regularized.pt')
        torch.save(self.state_dict(), file_path)
        print(f'Model has been saved to {file_path}')
    ...
    def load_net(self, load_path="saved_model/DT/dt.pt", device='cuda', critic_path=None):
        file_path = load_path
